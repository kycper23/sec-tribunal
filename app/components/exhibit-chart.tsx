...
export function ExhibitChart({ series }: { series: ChartSeries[] }) {
  const plottable = series.filter((s) => s.points.length >= 2)
  if (plottable.length === 0) return null

  // Union of fiscal periods across all series, sorted chronologically —
  // metrics can have slightly different coverage, so the x-axis is built
  // from whichever periods actually appear, not assumed to line up 1:1.
  const periods = [...new Set(plottable.flatMap((s) => s.points.map((p) => p.period)))].sort()
  const values = plottable.flatMap((s) => s.points.map((p) => p.value))
